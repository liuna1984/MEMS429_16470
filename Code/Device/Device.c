...
{	
	...
	g_hImu16470.NSS_GPIOx = CS_16470_GPIO_Port;
	g_hImu16470.NSS_GPIO_Pin = CS_16470_Pin;
	
	//	HAL_InitTick(1); // SYSTick 计时器1ms}	
	LED_TurnOnAll();
	HAL_Delay(1000);
  HAL_GPIO_WritePin(RST_16470_GPIO_Port, RST_16470_Pin, GPIO_PIN_RESET);
	HAL_Delay(1000);
	HAL_GPIO_WritePin(RST_16470_GPIO_Port, RST_16470_Pin, GPIO_PIN_SET);
	HAL_Delay(1000);
	//do
	{
	 ADI_16470_ReadDeviceNumber(&g_hImu16470);
	}	
	//while(g_hImu16470.nProductID != 0x4056);
	
	if (g_hImu16470.nProductID == 0x4056)
	{
		LED_Toggle(LED2);
	}	
